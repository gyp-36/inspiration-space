...
| 字段名              | 类型       | 长度  | 主键  | 非空  | 默认值               | 说明               |
| ---------------- | -------- | --- | --- | --- | ----------------- | ---------------- |
| session_id       | BIGINT   | -   | ✅   | ✅   | -                 | 会话ID（雪花ID）       |
| session_type     | TINYINT  | -   | -   | ✅   | 1                 | 1:群聊 2:单聊        |
| session_name     | VARCHAR  | 100 | -   | ❌   | ''                | 群聊名称；单聊可为空       |
| session_status   | TINYINT  | -   | -   | ✅   |                   | 会话状态：0正常，1封禁，2删除 |
| description      | VARCHAR  | 255 | -   | ❌   | ''                | 群描述（单聊忽略）        |
| session_avatar   | VARCHAR  | 255 | -   | ❌   | NULL              | 会话头像URL          |
...
- 普通索引：`idx_type (session_type)`
- 普通索引：`idx_created_at (created_at)`

---

##### 2. 会话成员表 (chat_session_members)
...
| content    | TEXT     | -   | -   | ✅   | -                 | 消息内容                  |
| file_url   | VARCHAR  | 512 | -   | ❌   | NULL              | 文件/图片URL              |
...
- 主键索引：`message_id`
- 复合索引：`idx_session_sent (session_id, sent_at)`
